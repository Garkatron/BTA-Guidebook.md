package deus.guidebookmd;

import deus.guidebookmd.item.ItemEditorBook;
import deus.guidebookmd.item.ItemGuidebookmd;
import net.minecraft.core.item.Item;
import turniplabs.halplibe.helper.ItemBuilder;
import turniplabs.halplibe.util.toml.Toml;

public class Items {

	public static Item MD_ITEM_GUIDE = null;
	public static Item MD_ITEM_EDITOR_BOOK = null;
	//public static Item MD_ITEM_DARK_GUIDE = null;
	private static final ItemBuilder genericItemBuilder = new ItemBuilder(Guidebookmd.MOD_ID);


	public static void initialize() {
		MD_ITEM_GUIDE = genericItemBuilder.build(new ItemGuidebookmd("guide",Guidebookmd.MOD_ID+":item/guide", Guidebookmd.CFG.getInt("IDs.guidebook")));
		MD_ITEM_EDITOR_BOOK = genericItemBuilder.build(new ItemEditorBook("editor_book",Guidebookmd.MOD_ID+":item/editor_book", Guidebookmd.CFG.getInt("IDs.guidebook")+1));
		//MD_ITEM_DARK_GUIDE = genericItemBuilder.build(new ItemGuidebookmd("dark_guide",Guidebookmd.MOD_ID+":item/dark_guide", 23002));
	}
}
